...
void persona::menu()
{
	...
	cout<<"\t\t\t-------------------------------"<<endl;
	cout<<"\t\t\t l   SISTEMA GESTION PERSONAS  l"<<endl;
	cout<<"\t\t\t-------------------------------"<<endl;
	...
	getch(); //pausa la secuencia y espera algo
    }while(choice!= 6);
}
void persona::insertar()
{
	...
	cin>>name;
	cout<<"\t\t\tIngresa Contrasena Persona   : ";
	cin>>pass;
...
void persona::desplegar()
{
	...
	else
	{
		...
		while(!file.eof())
		{
			...
			cout<<"\t\t\t Contrasena Persona: "<<pass<<endl;
			file >> id >> name >> pass;
		}
		if(total==0)
		{
	...
	else
	{
		...
		while(!file.eof())
		{
			if(participant_id==id)
			{
				cout<<"\n\n\t\t\t Id Persona: "<<id<<endl;
				cout<<"\t\t\t Nombre Persona: "<<name<<endl;
				cout<<"\t\t\t Contrase�a Persona: "<<pass<<endl;
				found++;
			}
			file >> id >> name >> pass;
		}
...
//append (adjuntar al final)
//out solo se usa cuando no hay archivo
